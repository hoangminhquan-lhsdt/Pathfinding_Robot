...
def Read_Map(file_name):
	...
	start = (int(points_content[0]), int(points_content[1]))
	goal = (int(points_content[2]), int(points_content[3]))
	...
	if points_content.__len__() > 4:
		...
		while i < points_content.__len__():
			pickup_points.append((int(points_content[i]), int(points_content[i+1])))
			i += 2
	...
	for index in range(N):
		...
		while i < obstacles_content.__len__():
			obstacle_vertex.append((int(obstacles_content[i]),int(obstacles_content[i+1])))
			i += 2
